...
parser = argparse.ArgumentParser("generic_analysis")
...
Shorten the question to consise format removing any extra words. Return valid JSON containing question and section, in format:\n
{{\n
...
RETRIEVAL_ANALYSIS_CORRECTION_PROMPT= """
...
def getRelevantDocsFromVectorStore(query, vector_store, top_documents_limit):
    print(f"DEBUG: Getting document matches from vector store")
    retrieved_docs = vector_store.similarity_search(query["query"], k=top_documents_limit)
...
def getBM25RankedDocs(query, bm25_index_data, top_documents_limit, already_retrieved_doc_ids):
    print(f"DEBUG: Getting document matches from ranking store")
    ...
    docs_as_text, bm25_scores = bm25_engine.retrieve(query_tokens, k=top_documents_limit)
    ...
    for idx, doc_text in enumerate(docs_as_text[0]):
        ...
        if original_index in already_retrieved_doc_ids:
            continue
        bm25_results.append({
            "id": original_index,
            "rank": idx,
...
def getRerankedDocs(reranker_llm, query, docs, top_documents_limit):
    print(f"DEBUG: Reranking documents with LLM")
    doc_texts = [doc.page_content for doc in docs]
    doc_rank_infos = reranker_llm.rank(query["query"], doc_texts, top_k=top_documents_limit)
    reranked_docs = []
    for doc_rank_info in doc_rank_infos:
        reranked_docs.append(docs[doc_rank_info["corpus_id"]])
    return reranked_docs
    
def getCorrectionCheckedDocs(correction_check_llm, query, docs_after_re_ranking):
    print(f"DEBUG: Filtering relevant documents with LLM")
    rag_prompt = PromptTemplate.from_template(RETRIEVAL_ANALYSIS_CORRECTION_PROMPT)
    correction_passed_docs = []
    for doc in docs_after_re_ranking:
        messages = rag_prompt.invoke({"question": query["query"], "document": doc.page_content})
    ...
    return correction_passed_docs
    
    
def retrieveContextFromStore(state, reranker_llm, correction_check_llm, vector_store, bm25_index_data):
    top_k = 10
    top_k_rerank = 5
    query = state["query"]
    retrieved_docs = getRelevantDocsFromVectorStore(query, vector_store, top_k)
    retrieved_doc_ids = []
    ...
    retrieved_ranker_docs = getBM25RankedDocs(query, bm25_index_data, top_k, retrieved_doc_ids)
    
        
    combined_docs = []
    if retrieved_ranker_docs:
        combined_docs += [item["doc"] for item in retrieved_ranker_docs]
    if retrieved_docs:
        combined_docs += retrieved_docs
    docs_after_re_ranking = getRerankedDocs(reranker_llm, query, combined_docs, top_k_rerank)
    final_docs_after_source_correction_check = getCorrectionCheckedDocs(correction_check_llm, query, docs_after_re_ranking)
    return {"context": final_docs_after_source_correction_check}
...
def main():
    print(f"DEBUG: starting to load dependency llm models")
    ...
    graph_builder = StateGraph(LLMState).add_sequence([
        ...
        ("retrieveContext", partial(retrieveContextFromStore, reranker_llm=reranker_llm, correction_check_llm=llm, vector_store=vector_store, bm25_index_data=bm25_index_data)),
