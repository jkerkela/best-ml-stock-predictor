    ...
    while True:
        try:
            if is_weekday():
                ...
                if shouldPost(time_on_last_IV_monitor_post, IV_CHECK_INTERVALI_IN_SECONDS) and (isNowInTimePeriod(dt_time(9, 30, 0), dt_time(11, 30, 0), current_time) or isNowInTimePeriod(dt_time(14, 00, 0), dt_time(16, 0, 0), current_time)):
            ...
            if shouldPost(time_on_last_trade_monitor_post, STOCK_TRADE_MONITOR_POST_INTERVAL_IN_SECONDS):
                print(f"Running tweet bot, current time in target timezone={current_time}")
